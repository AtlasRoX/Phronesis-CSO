...
- Complements `gwas-lookup` (germline), `opentargets-association-evidence` (somatic folded into the association `somatic_mutation` datatype), and the expression skills. Feeds the Target-ID division of `phronesis-cso`. Output is structured JSON, so it chains via the Bio Orchestrator.
