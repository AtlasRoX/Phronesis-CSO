...
- Fills the **FDA Safety Officer** role in `phronesis-cso` routing (Target Safety + Clinical), complementing `opentargets-target-factors` (structural/genetic safety) and `clinical-trial-finder` (trial outcomes). Drug names come from those skills. Output is structured JSON, so it composes via the Bio Orchestrator.
