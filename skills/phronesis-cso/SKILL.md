---
name: phronesis-cso
description: Multi-agent therapeutic-target-assessment orchestrator — a Chief-of-Staff briefing, decomposition + routing across four scientific divisions, a Scientific-Reviewer audit that can re-route to fill a gap, and a synthesized report; reproduces the loop from Phronesis CSO (Zhang et al. 2026) over ClawBio skills. The skill makes no LLM call; reasoning is delegated to the driving agent.
license: MIT
metadata:
  version: "0.1.0"
  ...
  endpoints:
    cli: python skills/phronesis-cso/cso.py --query {query} --output {output_dir}
  openclaw:
    requires:
      bins:
        - python3
