#!/usr/bin/env python
"""harness.py — run phronesis-cso as a LIVE multi-agent loop.
...
def run(query: str, out_dir: Path | None, *, backend: str, model: str | None,
    ...
    synthesis, _ = _agent_or_stub(
        ...
        syn_context, SYNTHESIS_SCHEMA,
        stub=cso.load_synthesis(query, case, results, demo=demo) or {}, rec=rec)
...
def _run_divisions(subtasks: list[cso.Subtask], runner: runners.Runner, query: str,
    ...
    """Phronesis structure: one **division scientist agent** per division.
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="harness.py",
        description="Run phronesis-cso as a live multi-agent loop "
