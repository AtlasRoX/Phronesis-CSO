"""Tests for the phronesis-cso skill (offline; no network/LLM)."""
import json
import os
import subprocess
import sys
...
def test_demo_writes_full_contract(tmp_path):
    ...
    envelope = json.loads((out / "result.json").read_text(encoding="utf-8"))
    assert envelope["skill"] == "phronesis-cso"
...
def test_demo_report_is_deterministic(tmp_path):
    ...
    assert (a / "report.md").read_text(encoding="utf-8") == (b / "report.md").read_text(encoding="utf-8")
...
from cso import _local_skill_args  # type: ignore # noqa: E402
