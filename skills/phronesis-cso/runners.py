...
``run_agent(prompt, context, schema)``, is backed by an auto-selected provider:
  ...
  - ``NIMRunner``       — NVIDIA NIM API (``NVIDIA_API_KEY``), Nemotron-3-Super-120B
                          via the OpenAI-compatible NIM endpoint.
...
SYSTEM = (
    "You are a division agent inside a Phronesis CSO multi-agent system. "
...
class NIMRunner:
    """NVIDIA NIM backend — Nemotron-3-Super-120B via the NIM OpenAI-compatible API.

    Uses ``NVIDIA_API_KEY`` for auth. Endpoint is the NVIDIA hosted inference platform.
    The model is accessed as ``nvidia/nemotron-3-super-120b-a12b`` via OpenAI
    client pointed at ``https://integrate.api.nvidia.com/v1``.
    """
    ...
    def __init__(self, model: str | None = None) -> None:
        ...
        if not key:
            raise NoBackendError("set NVIDIA_API_KEY to use the NVIDIA NIM backend")
        self.name = "nim"
        self.model = model or os.environ.get("VBIO_MODEL") or self.DEFAULT_MODEL
        self.last_usage: dict[str, int] = {}
...
def select_runner(backend: str = "auto", model: str | None = None) -> Runner:
    ...
    Order for ``auto``: Anthropic key, NVIDIA NIM key, then OpenAI key, then
    the Claude Code CLI (reuses local auth, no key), then a no-op StubRunner.
    The StubRunner is always returned (never None) so callers have a uniform
    object; it raises NoBackendError on use.

    Supported backends:
      auto         — try in order: anthropic, nim, openai, gemini, claude-cli, stub
      anthropic    — Anthropic Claude (ANTHROPIC_API_KEY)
      nim          — NVIDIA NIM Nemotron-3-Super-120B (NVIDIA_API_KEY)
      openai       — OpenAI-compatible (OPENAI_API_KEY + optional OPENAI_BASE_URL)
      gemini       — Google Gemini (GEMINI_API_KEY or GOOGLE_API_KEY)
      claude-cli   — Claude Code CLI (local auth, no key)
      stub         — offline stubs, no LLM call
    """
    backend = (backend or "auto").lower()
    has_anthropic = bool(os.environ.get("ANTHROPIC_API_KEY"))
    has_nim = bool(os.environ.get("NVIDIA_API_KEY"))
    ...
    if backend not in ("auto",):
        raise ValueError(
            f"unknown backend {backend!r} (use auto|anthropic|nim|openai|gemini|claude-cli|stub)")
