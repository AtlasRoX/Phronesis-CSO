...
metadata:
  ...
  outputs:
    ...
    - name: report
      ...
      format:
        - md
      description: Human-readable factor table grouped by Phronesis division.
  dependencies:
    python: ">=3.10"
  demo_data:
...
- Fills **Functional / Modality / Target-Safety** in the `phronesis-cso` routing, alongside `celltype-specificity-profiler` (single-cell specificity), `clinical-trial-finder` (trials), and `gwas-lookup` (germline). Output is structured JSON, so it composes via the Bio Orchestrator.
