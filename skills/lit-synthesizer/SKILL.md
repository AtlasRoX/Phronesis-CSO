...
- The natural **re-route** target in `phronesis-cso`: when the Scientific Reviewer flags missing recent-literature / competitive / emerging-safety context, it routes here. Output is structured, source-linked JSON, so it composes via the Bio Orchestrator and feeds the CSO synthesis with citable current evidence.
