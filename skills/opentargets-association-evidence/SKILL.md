...
- Complements `opentargets-target-factors` (target-level) and feeds the Target-ID / Clinical divisions of `phronesis-cso`. Pairs with `clinical-trial-finder` (the clinical evidence the OT `known_drug` datatype reflects). Output is structured JSON, so it chains via the Bio Orchestrator.
