...
const _icon = (phName) => ({ className = '', size, style, ...rest }) => {
  const sz = size ?? (
    className.includes('w-6') ? 24 :
    className.includes('w-3') ? 14 :
    className.includes('w-4') ? 16 : 20
  );
  // drop layout-only classes that don't apply to <i> (w-*, h-*, flex-none, etc.)
  const colorCls = className
    .split(' ')
    .filter(c => !c.startsWith('w-') && !c.startsWith('h-') && c !== 'flex-none')
    .join(' ')
    .trim();
      ...
      {g.edges.map((e,i)=>{
        ...
        return (
          <g key={"e"+i} className="cursor-pointer fade-up" onClick={(x)=>{x.stopPropagation();onEdge(i);}}>
            ...
            {sel && <text x={(a.x+b.x)/2} y={(a.y+b.y)/2-4} fill="#0f172a" fontSize="9" textAnchor="middle" className="mono">{e.type}</text>}
          </g>
        );
      })}
      {g.nodes.map(n=>{
        ...
        return (
          <g key={n.id} className="cursor-pointer fade-up" opacity={dim?0.28:1}
            ...
            <circle cx={p.x} cy={p.y} r={r} fill={st.fill} stroke={sel?"#0f172a":st.stroke} strokeWidth={sel?3:1.5}/>
            <text x={p.x} y={p.y+r+11} fill={sel?"#0f172a":"#475569"} fontSize="10" textAnchor="middle" style={{pointerEvents:"none"}}>
  ...
  if (selEdge != null && g.edges[selEdge]) {
    ...
    return (
      <div className="space-y-3">
        <div className="text-[11px] uppercase tracking-widest text-sky-800">Evidence · the edge IS the claim</div>
        <div className="mono text-xs text-slate-800">{sn?.label} <span className="text-sky-600">—{e.type}→</span> {tn?.label}</div>
        {e.value && <div className="text-base font-semibold text-slate-900">{e.value}</div>}
        <div className="flex items-center gap-2 flex-wrap">
          {e.axis && <Chip cls="border-sky-200 text-sky-700 bg-sky-50">{e.axis}</Chip>}
          {e.grade && <Chip cls={gradeStyle(e.grade)}>{e.grade}</Chip>}
          <Chip cls="border-slate-200 text-slate-700 bg-slate-100">{p.icon} {p.label.split(" — ")[0]}</Chip>
          {conf!=null && <Chip cls={gradeStyle(conf>=0.8?"strong":conf>=0.5?"supported":"suggestive")}>confidence {conf.toFixed(2)}</Chip>}
        </div>
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800 leading-relaxed">{e.ref || "—"}</div>
        <div className="text-xs text-slate-500">
          source: {e.url
            ? <a href={e.url} target="_blank" rel="noreferrer" className="text-sky-600 hover:text-sky-800 underline mono">{e.source || e.url} ↗</a>
            : <span className="mono text-slate-700">{e.source || "—"}</span>}
        </div>
        {e.step && stepsById[e.step] && (
          <div className="text-xs text-slate-500 border-t border-slate-200 pt-2">↳ from loop step <span className="mono text-slate-700">{stepsById[e.step].role}</span> — {stepsById[e.step].title}</div>
        )}
      </div>
    );
  }
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{background:st.fill}}></span>
          <span className="text-[11px] uppercase tracking-widest text-slate-600">{n.kind}</span></div>
        <div className="text-base font-semibold text-slate-900">{n.label}</div>
        {n.sub && <div className="text-xs text-slate-500">{n.sub}</div>}
        {n.shared && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            🔗 shared entity — also appears in {n.shared_runs.length} other hypothesis{n.shared_runs.length>1?"es":""}
            <div className="mono text-amber-700 mt-1">{n.shared_runs.join(", ")}</div>
          </div>
        )}
        {n.val!=null && (
          <div>
            <div className="text-[10px] uppercase tracking-wide text-slate-500 mb-1">normalized value</div>
            <div className="h-2 rounded-full bg-slate-200 overflow-hidden"><div className="h-full" style={{width:`${n.val*100}%`, background:st.fill}}></div></div>
            <div className="mono text-xs text-slate-600 mt-1">{n.val.toFixed(2)}</div>
          </div>
        )}
        {n.url && <a href={n.url} target="_blank" rel="noreferrer" className="inline-block text-xs text-sky-600 hover:text-sky-800 underline mono">{n.url} ↗</a>}
        <div className="border-t border-slate-200 pt-2">
          ...
          <ul className="space-y-1">{inc.map((e,i)=>{ const o=e.s===n.id?e.t:e.s; const on=g.nodes.find(x=>x.id===o);
            return <li key={i} className="text-xs text-slate-700 mono">{e.type} · {on?.label}</li>; })}</ul>
        </div>
        {step && <div className="text-xs text-slate-500 border-t border-slate-200 pt-2">↳ traces to <span className="mono text-slate-700">{step.role}</span> — {step.title}</div>}
      </div>
    );
  }
  return <div className="text-sm text-slate-500">Click a <span className="text-slate-800">node</span> for its normalized properties, or an <span className="text-slate-800">edge</span> to open its reference & source.</div>;
}
...
function LedgerView({ run }) {
  ...
  if (err) return <div className="text-sm text-rose-700">Could not load the ledger: {err}</div>;
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button onClick={load} className="px-3 py-1.5 rounded-lg text-xs font-medium text-slate-700 border border-slate-200 hover:bg-slate-50">↻ refresh</button>
      </div>

      <div className="text-xs text-slate-500">
        Accumulated across <span className="text-slate-700 font-semibold">every</span> query, persisted in <span className="mono">kg.json</span>. Each row traces to its source.
      </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="flex flex-wrap gap-2">
            {data.sources.map((s,i)=> s.url
              ? <a key={i} href={s.url} target="_blank" rel="noreferrer" className="text-xs text-sky-600 hover:text-sky-800 underline mono px-2 py-1 rounded border border-slate-200 bg-white">{s.label} ↗</a>
              : <span key={i} className="text-xs text-slate-500 mono px-2 py-1 rounded border border-slate-200 bg-white">{s.label}</span>)}
          </div>
        </div>
      )}

      {Object.entries(byHyp).map(([hyp, items])=>(
        <div key={hyp} className="rounded-2xl border border-slate-200 bg-slate-50/50 overflow-hidden">
          <div className="px-4 py-2 bg-slate-100 border-b border-slate-200 text-sm font-semibold text-slate-900 mono">{hyp || "—"}</div>
                ...
                return (
                  <tr key={i} className="border-t border-slate-200 align-top">
                    <td className="px-4 py-2 text-slate-800">
                      <span className="text-sky-600 mono">{r.relation}</span> → <span className="text-slate-900 font-medium">{r.object}</span>
                      <span className="ml-1 text-[10px] text-slate-500">{r.object_kind}</span>
                    </td>
                    <td className="px-2 py-2 text-slate-700">{r.value || "—"}</td>
                    <td className="px-2 py-2 text-slate-500">{r.axis || "—"}</td>
                    <td className="px-2 py-2"><Chip cls={gradeStyle(r.grade)}>{r.grade || "—"}</Chip></td>
                    <td className="px-2 py-2 mono text-slate-700">{r.conf!=null?Number(r.conf).toFixed(2):"—"}</td>
                    <td className="px-2 py-2">
                      <span className="mr-1">{p.icon}</span>
                      {r.url
                        ? <a href={r.url} target="_blank" rel="noreferrer" className="text-sky-600 hover:text-sky-800 underline">{r.source} ↗</a>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3 text-xs text-slate-600 flex-wrap">
          ...
          <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full border border-dashed border-amber-500"></span>shared across runs</span>
        ...
        <Chip cls={complete?"border-emerald-200 text-emerald-700 bg-emerald-50":"border-sky-200 text-sky-700 bg-sky-50"}>
          {complete ? `graph complete · ${g.nodes.length} nodes` : `building… ${g.nodes.length} nodes`}
        </Chip>
      </div>
      <div className="grid lg:grid-cols-[1fr_300px] gap-4">
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm p-2" onClick={()=>{setSelNode(null);setSelEdge(null);}}>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm p-4 self-start">
          <GraphInspector g={g} selNode={selNode} selEdge={selEdge} stepsById={stepsById}/>
        </div>
      </div>
      {!complete && <p className="text-xs text-slate-500">The graph grows as each division returns evidence; it's complete when the report is constructed.</p>}
    </div>
  );
}
    ...
    <div className="flex gap-4 fade-up">
      <div className="flex flex-col items-center">
        <div className={`w-9 h-9 rounded-full grid place-items-center text-sm font-bold border-2 transition
          ${running ? "border-sky-500 bg-sky-50 text-sky-700 pulse-ring"
          : step.terminal ? "border-emerald-500 bg-emerald-50 text-emerald-700"
          : step.reroute ? "border-amber-500 bg-amber-50 text-amber-700"
          : "border-slate-300 bg-slate-100 text-slate-600"}`}>
          {running ? "•" : step.terminal ? "✓" : idx+1}
        </div>
        {!last && <div className={`w-0.5 flex-1 my-1 ${step.reroute?"bg-amber-500/40":"bg-slate-200"}`} style={{minHeight:"1rem"}}/>}
      </div>
      <div className={`relative overflow-hidden flex-1 mb-3 rounded-xl border p-4 transition bio-panel ${kindCls} ${running?"ring-1 ring-cyan-500/40":""} ${active?"ring-1 ring-cyan-500/60":""} ${ev?"cursor-pointer hover:border-cyan-400":""}`} onClick={ev?onClick:undefined}>
        {running && <div className="absolute inset-x-0 top-0 h-[3px] bio-shimmer"/>}
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center gap-2">
            <Chip cls={step.kind==="agent"?"border-violet-200 text-violet-700 bg-violet-50":"border-slate-200 text-slate-700 bg-slate-100"}>{step.kind==="agent"?"AGENT":"SKILL"}</Chip>
            <span className="mono text-xs text-slate-500">{step.role}</span>
          </div>
          <div className="flex items-center gap-2">
            {running && <span className="text-xs text-sky-600">running…</span>}
            {ev && <Chip cls="border-slate-200 text-slate-700 bg-slate-50">{ev.provenance}</Chip>}
        ...
        <div className="text-xs text-slate-500">{step.division}</div>
        {metrics && (
          <div className="flex gap-4 mt-2">
            {Object.entries(metrics).map(([k,v])=> v!=null && (
              <div key={k} className="bg-slate-50 rounded-lg px-3 py-1.5 border border-slate-200">
                <div className="mono text-lg font-bold text-emerald-700">{v}</div>
                <div className="text-[10px] uppercase tracking-wide text-slate-500">{k}</div>
              </div>
            ))}
          </div>
        )}
        {ev && <p className="mt-2 text-sm text-slate-800 leading-relaxed">{res.summary || ev.digest}</p>}
        {step.review && (
          <div className="mt-2 text-sm text-slate-700">
            ...
            {(step.review.gaps||[]).map((gp,i)=><div key={i} className="text-xs text-amber-700 mt-1">↳ gap: {gp.missing} → re-route to {gp.route_to}</div>)}
          </div>
        )}
        {step.id==="review" && engineGaps && engineGaps.length>0 && (
          <div className="mt-3 rounded-lg border border-fuchsia-200 bg-fuchsia-50/50 p-3">
            <div className="flex items-center gap-2 text-xs">
              <span className="px-2 py-0.5 rounded-md font-bold bg-fuchsia-100 text-fuchsia-700 border border-fuchsia-200">◆ PROMETHEUX</span>
              <span className="text-fuchsia-800">deductive gap-detector · {engineForced ? "forced re-route" : "advisory"}</span>
            </div>
            {engineGaps.map((g,i)=>(
              <div key={i} className="mt-2 text-xs text-fuchsia-900">
        ...
        {step.id==="review" && panel && panel.lenses && panel.lenses.length>0 && (
          <div className="mt-3 relative overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-3.5 shadow-sm">
            <div className="text-[10px] uppercase tracking-[0.18em] text-slate-600 mb-2.5">4-lens reviewer panel</div>
            <div className="flex flex-wrap gap-1.5">
              {panel.lenses.map((l,i)=>(
                <span key={i} className={`px-2.5 py-1 rounded-lg text-xs font-medium border shadow-sm ${l.verdict==="re-route"?"text-rose-700 border-rose-200 bg-rose-50":"text-emerald-700 border-emerald-200 bg-emerald-50"}`}>
                  {l.key} {l.verdict==="re-route"?"✗ re-route":"✓"}
                </span>
              ))}
            </div>
            {panel.n_lenses!=null && (
              <div className="mt-2 text-xs text-slate-500">{panel.reroute_votes}/{panel.n_lenses} lenses flag re-route</div>
            )}
          </div>
        )}
        {active && ev && (res.top_cell_types) && (
          <table className="mt-3 w-full text-xs">
            ...
            <tbody>{res.top_cell_types.map((r,i)=><tr key={i} className="border-t border-slate-100"><td className="py-1 text-slate-700">{r.cell_type}</td><td className="mono text-slate-600">{r.mean_expr}</td><td className="mono text-slate-600">{(r.pct_expressing*100).toFixed(0)}%</td></tr>)}</tbody>
          </table>
        )}
        {active && ev && <div className="mt-3 text-xs text-slate-500"><span className="text-slate-500">reference: </span>{ev.reference}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <g onClick={clickable ? onClick : undefined} style={{ cursor: clickable ? "pointer" : "default" }}>
      ...
      <rect x={x} y={y} width={w} height={h} rx={11} fill={st.fill}
            ...
            opacity={step ? 1 : 0.45}/>
      {active && <rect x={x} y={y} width={w} height={h} rx={11} fill="none" stroke="#0284c7" strokeWidth="1.5" opacity="0.7"/>}
      <text x={x+w/2} y={y+h/2-2} textAnchor="middle" fontSize="11"
            fontFamily="'Space Grotesk',sans-serif" fontWeight="600"
            fill={step ? "#0f172a" : "#64748b"}>{label}</text>
...
function LoopGraph({ run, active, setActive }) {
  ...
  return (
    <div className="relative rounded-2xl border border-slate-200 shadow-sm"
      ...
      {/* overflow:visible on the SVG ensures nodes near the edge are never clipped by SVG viewport */}
      <svg viewBox={`0 0 ${W} ${H}`}
           style={{ display: "block", width: "100%", height: "auto", overflow: "visible" }}>
        <defs>
          <marker id="flowArr" viewBox="0 0 10 10" refX="8.5" refY="5"
                  markerWidth="7" markerHeight="7" orient="auto-start-reverse">
            <path d="M0 1 L9 5 L0 9" fill="none" stroke="#94a3b8"
                  strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
          </marker>
          <marker id="flowArrLoop" viewBox="0 0 10 10" refX="8.5" refY="5"
                  markerWidth="8" markerHeight="8" orient="auto-start-reverse">
            <path d="M0 1 L9 5 L0 9" fill="none" stroke={GSTATE.reroute.stroke}
                  strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/>
          </marker>
        </defs>

        {/* QUERY → CSO */}
        <FlowNode x={X.input} y={H/2 - 28} w={118} h={56}
                  step={run.meta ? {status:"done"} : null}
                  label="QUERY" sub="target question"/>
        {flow(`M${X.input + 118} ${H/2} H${X.cso - 4}`, "in-cso")}

        {/* CSO */}
        <FlowNode x={X.cso} y={csoY} w={csoW} h={csoH}
        ...
        <FlowNode x={briefX} y={briefY} w={briefW} h={briefH} step={briefStep}
        ...
        {divs.map((s, i) => (
          <React.Fragment key={"e" + i}>
            {flow(`M${X.cso + csoW} ${H/2} C${X.div - 70} ${H/2} ${X.div - 70} ${divY(i) + dH/2} ${X.div - 4} ${divY(i) + dH/2}`,
                  "cso-div" + i,
                  s.reroute ? GSTATE.reroute.stroke : "#475569",
                  s.reroute ? "5 4" : undefined)}
            <FlowNode x={X.div} y={divY(i)} w={dW} h={dH} step={s}
                      label={prettyDiv(s.division) + (s.reroute ? " ↺" : "")}
                      sub={s.role} active={active === s.id}
                      onClick={() => setActive(active === s.id ? null : s.id)}/>
          </React.Fragment>
        ))}
        {divs.length === 0 && (
          <FlowNode x={X.div} y={divY(0)} w={dW} h={dH}
                    step={null} label="divisions" sub="awaiting routing…"/>
        )}

        {/* divisions → Reviewer */}
        {flow(`M${X.div + dW} ${divY(Math.floor((dN-1)/2)) + dH/2} C${X.review - 60} ${H/2} ${X.review - 60} ${H/2} ${X.review - 4} ${H/2}`, "div-rev")}
        <FlowNode x={X.review} y={H/2 - revH/2} w={revW} h={revH} step={revStep}
                  label="Reviewer" sub="gap-gate · panel" active={active === "review"}
                  onClick={() => revStep && setActive(active === "review" ? null : "review")}/>

        {/* Reviewer → Synthesis */}
        {flow(`M${X.review + revW} ${H/2} H${X.synth - 4}`, "rev-synth")}
        <FlowNode x={X.synth} y={H/2 - synthH/2} w={synthW} h={synthH}
                  step={synthStep || (run.synthesis ? {status:"done"} : null)}
                  label="CSO" sub="synthesis"/>

        {/* Synthesis → Decision */}
        {flow(`M${X.synth + synthW} ${H/2} H${X.out - 4}`, "synth-out")}
        <FlowNode x={X.out} y={H/2 - outH/2} w={outW} h={outH}
                  step={run.decision && run.decision !== "PENDING"
                        ? {status:"done", terminal: run.decision === "GO"} : null}
                  label={decLabel} sub={run.decision === "PENDING" ? "awaiting" : "verdict"}/>

        {/* Re-route feedback arc */}
        {rerouted && (
          <>
            <path d={`M${X.review} ${H/2 + revH/2} C${X.review} ${H - 30} ${X.div} ${H - 30} ${X.cso + csoW/2} ${H - 30} L${X.cso + csoW/2} ${csoY + csoH + 3}`}
                  fill="none" stroke={GSTATE.reroute.stroke} strokeWidth="1.7" strokeDasharray="6 5"
                  markerEnd="url(#flowArrLoop)"/>
            <rect x={X.div - 10} y={H - 44} width={260} height={26} rx={13}
                  fill="#f8fafc" stroke={GSTATE.reroute.stroke} strokeOpacity="0.35"/>
            <text x={X.div + 120} y={H - 27} textAnchor="middle" fontSize="10"
      ...
      {active && run.steps.find(s => s.id === active) && (
        <div className="border-t border-slate-200 p-4">
          <LoopStep step={run.steps.find(s => s.id === active)}
                    ...
                    last active onClick={() => setActive(null)}
...
function LoopTrace({run}) {
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5 text-xs text-slate-500 flex-wrap">
        {Object.entries(PROV).map(([k,p])=><span key={k}>{p.icon} {p.label}</span>)}
        <div className="ml-auto inline-flex rounded-lg border border-slate-200 overflow-hidden bg-white">
          {[["graph","⬡ Process flow"],["timeline","☰ Timeline"]].map(([k,l])=>(
            <button key={k} onClick={()=>setView(k)}
              className={`px-3 py-1 text-xs transition ${view===k?"bg-slate-200 text-slate-800 font-semibold":"text-slate-600 hover:text-slate-800 hover:bg-slate-50"}`}>{l}</button>
          ))}
        </div>
      </div>
      ...
      {view==="timeline" && (
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
        {run.steps.map((s,i)=>(
              ...
              {Object.values(run.divisionFindings).map((df,i)=>{
                ...
                return (
                  <div key={i} className="flex items-start gap-2 text-xs">
                    <span className="text-slate-800 font-semibold whitespace-nowrap">{df.division}</span>
                    {f.evidence_grade && <Chip cls={gradeStyle(f.evidence_grade)}>{f.evidence_grade}</Chip>}
                    {f.interpretation && <span className="text-slate-600 truncate">{f.interpretation}</span>}
                  </div>
                );
              })}
            </div>
...
function PrometheuxDecision({run, className}) {
  ...
  const axes = e.axes || {};
  return (
    <div className={`relative overflow-hidden rounded-2xl border border-fuchsia-200 bg-fuchsia-50/60 shadow-sm p-5 ${className||""}`}>
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <span className="px-2 py-0.5 rounded-md font-bold text-xs bg-fuchsia-100 text-fuchsia-700 border border-fuchsia-200">◆ PROMETHEUX</span>
        <span className="text-xs uppercase tracking-widest text-fuchsia-700">deductive decision</span>
        <span className={`ml-auto px-3 py-1 rounded-lg font-extrabold text-sm uppercase ${dec.c}`}>{(e.tier||"REVIEW").replace("_"," ")}</span>
      </div>
      <div className="text-sm text-slate-700">
        coverage score <span className="mono font-bold text-fuchsia-700">{e.score}</span>
      ...
      <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
        {Object.entries(axes).map(([ax,a])=>{
          ...
          return (
          <div key={ax} className={`rounded-lg border px-3 py-2 ${absent?"border-rose-200 bg-rose-50 border-dashed":"border-slate-200 bg-white"}`}>
            <div className="text-[10px] uppercase tracking-wide text-slate-500">{ax}</div>
            <div className={`text-sm font-semibold ${a.weight>=1?"text-emerald-700":a.weight>=0.5?"text-sky-700":a.weight>0?"text-amber-700":"text-rose-700"}`}>{absent?"no information":a.grade}</div>
      ...
      {e.explanation && <p className="mt-3 text-xs text-fuchsia-800 leading-relaxed border-t border-fuchsia-200 pt-3">{e.explanation}</p>}
      {run.diverges && (
        <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 flex items-start gap-2">
          <Icons.AlertTriangle className="w-4 h-4 text-amber-600 flex-none mt-0.5" />
          <div>
            <b>Divergence:</b> the synthesis agent proposed <b>{(run.agentDecision||"").replace("_"," ")}</b>, but the deductive layer derives <b>{(e.tier||"").replace("_"," ")}</b> from the evidence coverage. The derived tier is the decision of record.
          </div>
        </div>
      )}
    </div>
...
function Panel({title, accent, children}) {
  ...
  return (
    <div className={`rounded-2xl border ${bg} shadow-sm p-5`}>
      <div className={`text-xs uppercase tracking-widest mb-3 flex items-center gap-1.5 font-bold ${a}`}>
...
function AxisEvidence({ ax, entry }) {
  ...
  return (
    <div className="space-y-4">
      <div className={`rounded-2xl border p-5 ${absent ? "border-rose-200 bg-rose-50" : "border-slate-200 bg-white shadow-sm"}`}>
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs uppercase tracking-widest text-slate-600">{ax}</span>
          {a && <Chip cls={gradeStyle(absent ? "absent" : a.grade)}>{absent ? "no information" : a.grade}</Chip>}
          {a && <Chip cls="border-slate-200 text-slate-700 bg-slate-100">weight {a.weight}</Chip>}
          <span className="ml-auto text-xs text-slate-500">{entry.rows.length} evidence item{entry.rows.length!==1?"s":""}</span>
        </div>
        {absent && <p className="mt-2 text-xs text-rose-700">No information on this axis was gathered — the score reflects absence, not weak evidence.</p>}
      </div>
      {entry.rows.length > 0 && (
            ...
            <tbody>
              {entry.rows.map((r,i)=>{
                ...
                return (
                  <tr key={i} className="border-t border-slate-200 align-top">
                    <td className="px-4 py-2 text-slate-800">
                      <span className="text-sky-600 mono">{r.type}</span> → <span className="text-slate-900 font-medium">{r.objectLabel}</span>
...
function ReportOverview({run, s}) {
  ...
  return (
    <div className="space-y-5">
      <div className={`relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm p-6 ring-1 ${dec.ring}`}>
        <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 mb-3">Executive summary · CSO verdict</div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {run.decisionSource==="prometheux" && <Chip cls="border-fuchsia-200 text-fuchsia-700 bg-fuchsia-50">◆ derived</Chip>}
          <Chip cls="border-slate-200 text-slate-700 bg-slate-100 mono">confidence: {s.confidence||run.confidence}</Chip>
        </div>
        <p className="mt-4 text-sm text-slate-800 leading-relaxed">{s.recommendation}</p>
        {s.target_overview && <p className="mt-3 text-xs text-slate-600 leading-relaxed border-t border-slate-200 pt-3">{s.target_overview}</p>}
      </div>
      {run.decisionEngine && <PrometheuxDecision run={run}/>}
      <div className="grid md:grid-cols-2 gap-5">
        <Panel title="Liabilities & risks" accent="rose">
          {(s.liabilities||[]).map((l,i)=><div key={i} className="mb-3 last:mb-0"><div className="text-sm text-slate-800">{l.risk}</div>{l.mitigation && <div className="text-xs text-emerald-700 mt-0.5">↳ mitigation: {l.mitigation}</div>}</div>)}
        </Panel>
        <Panel title="Evidence gaps" accent="amber">
          <ul className="space-y-2">{(s.evidence_gaps||[]).map((g,i)=><li key={i} className="text-sm text-slate-700 flex gap-2">⚪ {g}</li>)}</ul>
        </Panel>
      </div>
      {(s.proposed_experiments||[]).length>0 && (
        <Panel title="Proposed experiments" accent="sky">
          <div className="grid sm:grid-cols-2 gap-3">
            {s.proposed_experiments.map((e,i)=><div key={i} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <div className="text-sm font-semibold text-slate-900">{e.experiment}</div>
              {e.expected_readout && <div className="text-xs text-sky-700 mt-1">readout: {e.expected_readout}</div>}
              {e.rationale && <div className="text-xs text-slate-600 mt-1">{e.rationale}</div>}
            </div>)}
          </div>
        </Panel>
      )}
  ...
  if (run.status !== "done" || !s) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center">
        <div className="text-sm text-slate-600">The report is constructed once the loop completes.</div>
        <div className="text-xs text-slate-400 mt-1">{run.status==="running" ? "synthesis pending — evidence still being gathered…" : "submit a query to begin."}</div>
      </div>
    );
  }
        ...
        {axisKeys.map(ax=>{
          ...
          return (
            <button key={ax} onClick={()=>setSub(ax)}
              ...
              <span className={`text-[10px] ${active===ax?"opacity-80":absent?"text-rose-600":"text-slate-500"}`}>
...
function QueryScreen({onRun}) {
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="hidden lg:flex flex-col justify-between w-2/5 min-w-[340px] max-w-[520px] bg-slate-900 px-12 py-14 text-white">
        <div>
          <div className="inline-flex items-center gap-2 text-[11px] text-sky-300 mono mb-8 px-2.5 py-1 rounded-full border border-sky-500/30 bg-sky-500/10">
            <span className="w-1.5 h-1.5 rounded-full bg-sky-400 bio-breath"/>phronesis-cso &middot; multi-agent harness
          </div>
          <h1 className="text-4xl xl:text-5xl font-extrabold leading-tight tracking-tight mb-5">
            Ask the<br/>Virtual CSO.
          </h1>
          <p className="text-slate-400 text-sm leading-relaxed">
            Submit a target-assessment question. A Chief-of-Staff briefing, division scientists, a four-lens Scientific Reviewer panel with a bounded re-route loop, and a CSO synthesis run in real time.
          </p>
          <div className="mt-10 grid grid-cols-2 gap-3">
            ...
            ].map(({ icon: IconComponent, color, title, desc }) => (
              <div key={title} className="rounded-xl border border-slate-800 bg-slate-900/50 p-3.5 flex flex-col gap-2">
        ...
        <div className="text-[11px] text-slate-600 mono">
          Live multi-agent loop via server.py → harness.py / cso.py
        </div>
      </div>

      {/* RIGHT PANEL — form */}
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-xl mx-auto px-8 py-14 fade-up">
          {/* mobile badge */}
          <div className="flex lg:hidden items-center gap-2 text-[11px] text-sky-700 mono mb-6 px-2.5 py-1 rounded-full border border-sky-200 bg-sky-50 w-fit">
            <span className="w-1.5 h-1.5 rounded-full bg-sky-500 bio-breath"/>phronesis-cso
          </div>
          <h2 className="text-2xl font-extrabold text-slate-900 mb-1">New assessment</h2>
          <p className="text-sm text-slate-500 mb-7">Enter your target–indication question below.</p>

          <form onSubmit={(e)=>{e.preventDefault(); if(q.trim()) onRun(q.trim(), demo, partial, agents, BUDGETS.find(b=>b.key===budget).tokens, hitl);}}>
            ...
            <div className="mt-5 space-y-3">
              <label className="flex items-start gap-3 cursor-pointer group">
                <input type="checkbox" checked={demo} onChange={(e)=>setDemo(e.target.checked)} className="accent-sky-500 mt-0.5"/>
                <span className="text-sm"><span className="text-slate-800 font-medium">Demo mode</span> <span className="text-slate-400">&mdash; cached data for all skills, reliable for a demo stage</span></span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={agents} onChange={(e)=>setAgents(e.target.checked)} className="accent-emerald-500 mt-0.5"/>
                <span className="text-sm"><span className="text-emerald-700 font-medium">Live agents</span> <span className="text-slate-400">&mdash; reasoning roles call a real LLM (~3-4 min). Off = instant stubs.</span></span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={partial} onChange={(e)=>setPartial(e.target.checked)} className="accent-fuchsia-500 mt-0.5"/>
                <span className="text-sm"><span className="text-fuchsia-700 font-medium">Skip safety step</span> <span className="text-slate-400">&mdash; demo the Prometheux gap-detector forcing a re-route</span></span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" checked={hitl} onChange={(e)=>setHitl(e.target.checked)} className="accent-amber-500 mt-0.5"/>
                <span className="text-sm"><span className="text-amber-700 font-medium">Human in the loop</span> <span className="text-slate-400">&mdash; pause at each reviewer pass to approve, override, or add a gap</span></span>
              </label>
            </div>

            <button type="submit" className="mt-7 w-full py-3 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-semibold text-sm shadow-md shadow-sky-100 transition">
              Run assessment →
            </button>
          </form>

          <div className="mt-8 border-t border-slate-100 pt-6">
            <div className="text-[11px] uppercase tracking-wide text-slate-400 mb-3">Try an example</div>
            <div className="flex flex-col gap-2">
              {EXAMPLES.map((ex,i)=><button key={i} onClick={()=>setQ(ex)} className="text-left text-sm text-slate-700 rounded-lg border border-slate-200 hover:border-sky-400 hover:text-slate-900 bg-white hover:bg-sky-50 px-3 py-2.5 transition shadow-sm">{ex}</button>)}
            </div>
          </div>
        </div>
      </div>
...
function CheckpointModal({ cp, onDecide }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm fade-up">
      <div className="relative overflow-hidden max-w-lg w-full mx-4 rounded-2xl border border-amber-200 bg-white shadow-2xl p-6 ring-1 ring-amber-300/30">
        <div className="text-xs text-amber-700 mono mb-1">🧑‍⚖️ human in the loop · reviewer pass {(cp.iteration ?? 0) + 1}</div>
        <h3 className="text-lg font-bold text-slate-900">The panel voted <span className={verdict==="re-route"?"text-amber-600":"text-emerald-600"}>{verdict}</span>.</h3>
        <p className="text-sm text-slate-600 mt-1">
          {cp.panel ? `${cp.panel.reroute_votes}/${cp.panel.n_lenses} lenses flagged a re-route. ` : ""}
          {pr ? <>Proposed next step: <span className="text-slate-800 mono">{pr.skill}</span>{pr.missing?` — to fill "${pr.missing}"`:""}.  </> : "No follow-up proposed."}
        </p>
        {(cp.gaps && cp.gaps.length>0) && (
          <ul className="mt-3 text-xs text-slate-500 list-disc pl-5 space-y-0.5">
            {cp.gaps.slice(0,4).map((g,i)=><li key={i}><span className="text-slate-800">{g.missing}</span>{g.route_to?` → ${g.route_to}`:""}</li>)}
          </ul>
        )}
        <div className="mt-5 grid grid-cols-2 gap-2">
          <button onClick={()=>onDecide({action:"approve"})}
            className="px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold">&#10003; Approve verdict</button>
          {verdict==="re-route"
            ? <button onClick={()=>onDecide({action:"override_verdict",verdict:"synthesize"})}
                className="px-3 py-2 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-800 text-sm font-semibold">■ Stop &amp; synthesize</button>
        ...
        <div className="mt-4 border-t border-slate-200 pt-4">
          ...
          <div className="flex gap-2 mt-2">
            <button disabled={!skill} onClick={()=>onDecide({action:"redirect",route_to:skill,missing:missing||undefined})}
              className="flex-1 px-3 py-1.5 rounded-lg border border-amber-400 text-amber-700 bg-amber-50 hover:bg-amber-100 disabled:opacity-40 text-xs font-semibold">→ Redirect re-route</button>
            <button disabled={!skill&&!missing} onClick={()=>onDecide({action:"add_gap",route_to:skill||undefined,missing:missing||undefined})}
              className="flex-1 px-3 py-1.5 rounded-lg border border-sky-400 text-sky-700 bg-sky-50 hover:bg-sky-100 disabled:opacity-40 text-xs font-semibold">+ Add gap to chase</button>
          </div>
        </div>
      </div>
    </div>
...
function App() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* FIXED TOP BAR */}
      <header className="flex-none flex items-center gap-4 px-5 border-b border-slate-200 bg-white shadow-sm" style={{height:"56px"}}>
        <button onClick={()=>{ if(esRef.current) esRef.current.close(); setRun(emptyRun()); }}
        ...
        <h1 className="flex-1 text-sm font-semibold text-slate-900 truncate">{run.meta?.query}</h1>
        <div className="flex-none flex items-center gap-2">
          <Chip cls={run.status==="done"?"border-emerald-200 text-emerald-700 bg-emerald-50":run.status==="error"?"border-rose-200 text-rose-700 bg-rose-50":"border-sky-200 text-sky-700 bg-sky-50"}>
            {run.status==="running"?"● running":run.status==="done"?"complete":"error"}
          </Chip>
          {run.meta?.partial && <Chip cls="border-fuchsia-200 text-fuchsia-700 bg-fuchsia-50">◆ partial</Chip>}
        </div>
        <div className="w-px h-5 bg-slate-200"/>
        {/* stat pills */}
        <div className="flex-none flex items-center gap-4 text-xs">
          <span className="flex items-center gap-1 text-slate-600">
            ...
            <span>steps: <span className="font-bold text-slate-900 mono">{stepsDone}</span></span>
          </span>
          <span className="text-slate-200">|</span>
          <span className="flex items-center gap-1 text-slate-600">
            <Icons.Database className="w-3.5 h-3.5 text-slate-400" />
            <span>evidence: <span className="font-bold text-slate-900 mono">{Object.keys(run.evidence).length}</span></span>
          </span>
          <span className="text-slate-200">|</span>
          <span className={`px-2.5 py-0.5 rounded-lg font-bold text-xs uppercase ${dec.c}`}>{decTier.replace("_"," ")}</span>
        </div>
      </header>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* FIXED LEFT SIDEBAR */}
        <aside className="flex-none flex flex-col border-r border-slate-200 bg-slate-50" style={{width:"180px"}}>
          <nav className="flex-1 py-3">
            {TAB_ITEMS.map(({k,icon:IconComponent,label,pending})=>(
              <button key={k} onClick={()=>setTab(k)}
                ...
                {pending && <span className="text-[9px] text-slate-400">soon</span>}
              </button>
            ))}
          </nav>
          {/* prov legend */}
          <div className="border-t border-slate-200 px-3 py-3">
            <div className="text-[9px] uppercase tracking-widest text-slate-400 mb-2">Sources</div>
            {Object.entries(PROV).map(([k,p])=>(
              <div key={k} className="flex items-center gap-1.5 text-[10px] text-slate-500 mb-1">
                <span>{p.icon}</span><span className="truncate">{p.label.split(" —")[0]}</span>
              </div>
            ))}
          </div>
          {/* run meta */}
          <div className="border-t border-slate-200 px-3 py-2.5">
            <div className="text-[10px] text-slate-400 mono truncate">
              {run.meta?.calls_llm
                ? `${run.meta.backend === "nim" ? "🟢 NIM" : run.meta.backend} · ${run.meta.model?.split("/").pop() || run.meta.model}`
                : "demo / stub"}
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0 overflow-auto">
          {run.status==="error" && (
            <div className="m-5 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 flex items-start gap-2">
              <Icons.AlertTriangle className="w-5 h-5 text-rose-600 flex-none" />
              <span>{run.error}</span>
            </div>
          )}
          <div className="p-6">
...
function Stat({n,l,small}){return <div className="px-3 py-2 rounded-xl bg-white border border-slate-200 shadow-sm"><div className={`font-bold text-slate-900 ${small?"text-sm":"text-xl"} ${small?"":"mono"}`}>{n}</div><div className="text-[10px] uppercase tracking-wide text-slate-500">{l}</div></div>;}
