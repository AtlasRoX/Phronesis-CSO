#!/usr/bin/env python3
"""server.py — live backend for the Phronesis CSO frontend.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _stream(self, qs):
        ...
        if agents in ("1", "true"):
            import os
            import shutil
            has_llm = any(os.environ.get(k) for k in ("ANTHROPIC_API_KEY", "NVIDIA_API_KEY", "OPENAI_API_KEY", "GEMINI_API_KEY", "GOOGLE_API_KEY")) or bool(shutil.which("claude"))
            if has_llm:
                backend = CONFIG["backend"] if CONFIG["backend"] != "stub" else "auto"
                live, demo = True, False
            else:
                backend = "stub"
                live, demo = False, True
...
def main():
    p = argparse.ArgumentParser(description="Live server for the Phronesis CSO frontend.")
