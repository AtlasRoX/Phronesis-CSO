...
The demo case study for the hybrid Phronesis CSO. Reproduces, in scoped form, the
B7-H3 analysis from Zhang et al. 2026 — showing the orchestration end-to-end on one query.

Packaged for ClawBio as the **`phronesis-cso`** skill (PR #2), which sits below
...
- `clawbio run phronesis-cso --demo` runs the whole chain **fully offline** from cached,
...
clawbio run phronesis-cso --query "Assess B7-H3 potential as a therapeutic target in lung cancer" --live
```

Source: [`skills/phronesis-cso/`](../skills/phronesis-cso/).
